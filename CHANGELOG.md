...
#### デバイス固有パラメータの最適化（二重増幅問題の修正）

**問題**:
- v1.3.6以降、iPhoneで音量バーがすぐに100%に到達してしまう
- 音域テスト開始時にノイズが検出されてしまう
- iPadでも同様に音量が飽和する傾向

**原因**:
- 二重増幅問題: `sensitivity × RMS_TO_PERCENT(100) × volumeMultiplier` の計算で過度な増幅
- iPhone: 3.5 × 100 × 3.0 = 1050倍の増幅
- iPad: 4.0 × 100 × 4.0 = 1600倍の増幅

**修正内容**:
DeviceDetection.tsのデバイス固有パラメータを最適化：

| デバイス | sensitivity | noiseGate | volumeMultiplier | 変更理由 |
|---------|------------|-----------|------------------|----------|
| **iPhone** | 3.5→**2.0** | 0.028→**0.08** | 3.0→**2.0** | 環境ノイズ増幅抑制、音域テストノイズ対策 |
| **iPad** | 4.0→**2.5** | 0.023→**0.05** | 4.0→**3.0** | ノイズフロア低減、100%飽和防止 |
| **PC** | 1.7 | 0.023 | 2.5 | 変更なし（安定動作） |

**影響**:
- ✅ iPhone/iPadで音量バーが自然な挙動に改善
- ✅ 静寂時の誤検知（ノイズフロア）が減少
- ✅ 音域テスト開始時のノイズ対策強化
- ✅ 50%程度の音量で100%に到達する問題を解消

**関連ファイル**:
- `/src/utils/DeviceDetection.ts`: getDeviceOptimizations()のパラメータ調整
