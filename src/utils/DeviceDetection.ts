...
export class DeviceDetection {
  ...
  private static getDeviceOptimizations(deviceType: 'iPhone' | 'iPad' | 'PC', _isIOS: boolean) {
    switch (deviceType) {
      case 'iPad':
        return {
          sensitivity: 7.0,           // 🎤 マイク感度 (5.0→7.0で低い声対応)
          noiseGate: 0.010,           // 🚪 ノイズゲート閾値 (0.020→0.010で低音拾いやすく)
          volumeMultiplier: 15.0,     // 🔊 表示音量補正 (18.0→15.0で安定重視)
          smoothingFactor: 0.20       // 📊 平滑化係数 (0.25→0.20で反応向上)
        };
