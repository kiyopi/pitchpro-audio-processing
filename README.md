...
## 🎯 最新リリース：v1.3.3 パフォーマンス最適化と音量バー安定化

### 🚀 パフォーマンス最適化（3項目）

**1. CPU負荷軽減**
- **smoothingFactor**: 0.25 → 0.1（全デバイス）
- **効果**: CPU負荷 -15%削減、より応答性の高い検出

**2. FPS安定化**
- **フレーム落ち判定閾値**: 1.5倍 → 2.0倍
- **効果**: 一時的な負荷変動を許容し、60 FPS安定維持

**3. データ収集増加**
- **初期FPS**: 45 → 60
- **効果**: データ収集 +27%（14.5件 → 18.5件/セッション）

### 🔧 音量バー安定化

**4. ノイズゲート調整**
- **閾値倍率**: 10倍 → 2.0倍
- **効果**: 音量バーの突然の0表示を防止、安定した表示を実現

### 📊 実機テスト結果

| 指標 | v1.3.2 | v1.3.3 | 改善 |
|------|--------|--------|------|
| データ収集 | 14.5件 | 18.5件 | +27% |
| FPS | 不安定 | 60 FPS安定 | ✅ |
| 音量バー | 突然0 | 正常動作 | ✅ |
| CPU負荷 | 高い | 軽減 | -15% |
...
**🎯 対象ユーザー**：全ユーザー（パフォーマンス向上のため更新推奨）

```bash
# 最新版へのアップデート
npm install @pitchpro/audio-processing@1.3.3
```
