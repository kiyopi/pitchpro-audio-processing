...
### 📱 クロスプラットフォーム最適化
- **デバイス別最適化設定**（v1.3.12）：
  - PC: noiseGate 2.3%、volumeMultiplier 2.5x（安定動作）
  - iPhone: noiseGate 8.0%、volumeMultiplier 2.0x（ノイズ対策強化）
  - iPad: noiseGate 5.0%、volumeMultiplier 3.0x（バランス調整）
...
```bash
# 最新版へのアップデート
npm install @pitchpro/audio-processing@1.3.12
```
...
#### v1.3.11 - デバイス固有パラメータ最適化
- **🔧 二重増幅問題の修正（部分的）**: DeviceDetectionのパラメータ調整
- **📱 iPhone**: sensitivity 3.5→2.0、noiseGate 2.8%→8.0%、volumeMultiplier 3.0→2.0
- **📱 iPad**: sensitivity 4.0→2.5、noiseGate 2.3%→5.0%、volumeMultiplier 4.0→3.0
- **⚠️ 注意**: RMS_TO_PERCENT_FACTORの修正漏れによりv1.3.12で完全修正

#### v1.3.6 - setCallbacks()音量値修正
- **🐛 バグ修正**: `setCallbacks()`の`onPitchUpdate`が処理済み音量を返すように修正
- **✅ 効果**: `result.volume`がUI表示と一致（0-100%範囲）
...
- **📈 FPS安定化**: フレーム落ち判定閾値 1.5倍 → 2.0倍
- **📊 データ収集増加**: 初期FPS 45 → 60（+27%向上）
- **🔧 音量バー安定化**: ノイズゲート閾値倍率 10倍 → 2.0倍

#### v1.3.2 - バックグラウンド/フォアグラウンド切り替え最適化
- **🔧 ページ表示状態切り替え時の安定性向上**：ブラウザタブ切り替え時の挙動を大幅改善
- **📊 バックグラウンド時**：健全性チェックを完全停止してリソース消費を削減
- **🔄 フォアグラウンド復帰時**：自動復旧試行回数をリセットして確実に復旧
- **🚫 エラーループ防止**：不要なエラー発生を抑制

#### v1.3.1 - PC低周波数検出回復
- **🔧 PC環境100Hz以下検出回復**：過度なノイズ対策による低周波数ブロック問題を解決
- **📊 音量バー更新統一化**：キャッシュ要素優先によるクロスモード干渉完全防止
- **🔬 段階的最適化手法**：5.0% → 3.5% → 2.3%による科学的調整確立
- **🎸 低周波数楽器対応**：ベース・バリトンサックス・チェロ等の安定検出実現

#### v1.3.0 - iPhone音響検出最適化
- **🎯 30Hz低周波数ノイズ解決**：iPhone特有の低周波数ノイズ問題の完全解決
...
#### 📥 直接ダウンロード
- **最新版**: [v1.3.12 リリース](https://github.com/kiyopi/pitchpro-audio-processing/releases/latest)
- **UMDファイル**: [pitchpro.umd.js](https://github.com/kiyopi/pitchpro-audio-processing/releases/download/v1.3.12/pitchpro.umd.js)
...
### デバイス別最適化パラメータ（v1.3.12）

| デバイス | sensitivity | noiseGate | volumeMultiplier | 特別対応 |
|----------|-------------|-----------|------------------|----------|
| **PC** | 1.7 | 2.3% | 2.5x | 安定動作 |
| **iPhone** | 2.0 | 8.0% | 2.0x | ノイズ対策強化 |
| **iPad** | 2.5 | 5.0% | 3.0x | バランス調整 |
