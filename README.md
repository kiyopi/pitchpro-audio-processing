...
## 🎯 最新リリース：v1.3.2 バックグラウンド/フォアグラウンド切り替え最適化

### 🔧 ページ表示状態切り替え時の安定性向上
**MicrophoneLifecycleManager最適化**：ブラウザタブ切り替え時の挙動を大幅改善
- **バックグラウンド時**：健全性チェックを完全停止してリソース消費を削減
- **フォアグラウンド復帰時**：自動復旧試行回数をリセットして確実に復旧
- **エラーループ防止**：不要なエラー発生を抑制

### 📊 技術的改善
**handleVisibilityChange()メソッド強化**：
- ページ非表示時：stopAllMonitoring()による完全なモニタリング停止
- ページ表示時：autoRecoveryAttemptsリセット + isActive復旧ロジック
- 1秒後の健全性チェック実行による安全な復帰

**継続中のデバイス最適化設定（v1.3.1より）**：
- **PC**: noiseGate 2.3%（低周波数検出回復）、volumeMultiplier 7.5x
- **iPhone**: noiseGate 2.8%（30Hz低周波数対応）、volumeMultiplier 9.0x
- **iPad**: noiseGate 2.3%（感度向上）、volumeMultiplier 13.0x

**🎯 対象ユーザー**：タブ切り替えを頻繁に行うユーザー、モバイルデバイスユーザー

```bash
# 最新版へのアップデート
npm install @pitchpro/audio-processing@1.3.2
```
